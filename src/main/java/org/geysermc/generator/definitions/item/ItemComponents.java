...
public record ItemComponents(RuntimeItemStates states, Map<Identifier, CompoundTag> components) {
    ...
    public static CompletableFuture<ItemComponents> open(MappingAccess access) {
        return access.readJsonFile(FileType.RUNTIME_ITEM_STATES).thenCombine(access.readNbtFile(FileType.ITEM_COMPONENTS_PALETTE), ItemComponents::new);
    }
}
